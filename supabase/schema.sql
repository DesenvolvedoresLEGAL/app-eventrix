...
COMMENT ON FUNCTION "public"."create_default_tenant_settings"("tenant_uuid" "uuid") IS 'Cria configurações padrão para novos tenants';
...
COMMENT ON FUNCTION "public"."get_current_tenant_id"() IS 'Extrai tenant_id do JWT token';
...
COMMENT ON FUNCTION "public"."log_tenant_changes"() IS 'Registra mudanças para auditoria e compliance LGPD';
...
COMMENT ON COLUMN "public"."subscription_plans"."max_events_max" IS '-1 indica ilimitado';



COMMENT ON COLUMN "public"."subscription_plans"."max_visitors_max" IS '-1 indica ilimitado';
...
CREATE OR REPLACE VIEW "public"."tenant_overview" AS
 SELECT "t"."id",
    ...
    "t"."max_exhibitors_allowed",
        CASE
            WHEN (("t"."max_events_allowed" = '-1'::integer) OR ("t"."max_events_allowed" >= 999999)) THEN (0)::numeric
            ELSE "round"(((("t"."current_events_count")::numeric / ("t"."max_events_allowed")::numeric) * (100)::numeric), 2)
        END AS "events_usage_percentage",
        CASE
            WHEN (("t"."max_admins_allowed" = '-1'::integer) OR ("t"."max_admins_allowed" >= 999999)) THEN (0)::numeric
            ELSE "round"(((("t"."current_admins_count")::numeric / ("t"."max_admins_allowed")::numeric) * (100)::numeric), 2)
    ...
    "t"."trial_ends_at",
        ...
        CASE
            ...
            WHEN ("t"."trial_ends_at" > CURRENT_TIMESTAMP) THEN (EXTRACT(days FROM ("t"."trial_ends_at" - CURRENT_TIMESTAMP)))::integer
            ELSE 0
        END AS "trial_days_remaining"
   FROM ((((("public"."tenants" "t"
     JOIN "public"."organizer_types" "ot" ON (("t"."organizer_type_id" = "ot"."id")))
     JOIN "public"."business_segments" "bs" ON (("t"."primary_segment_id" = "bs"."id")))
     JOIN "public"."tenant_statuses" "ts" ON (("t"."status_id" = "ts"."id")))
     JOIN "public"."subscription_plans" "sp" ON (("t"."plan_id" = "sp"."id")))
     JOIN "public"."brazilian_states" "st" ON (("t"."state_id" = "st"."id")))
  WHERE ("t"."deleted_at" IS NULL);


ALTER TABLE "public"."tenant_overview" OWNER TO "postgres";


COMMENT ON VIEW "public"."tenant_overview" IS 'Visão completa dos tenants com métricas e informações agregadas';



CREATE OR REPLACE VIEW "public"."active_tenants_summary" AS
 SELECT "count"(*) AS "total_active_tenants",
    "count"(*) FILTER (WHERE ("tenant_overview"."is_in_trial" = true)) AS "tenants_in_trial",
    "count"(*) FILTER (WHERE (("tenant_overview"."status")::"text" = 'Ativo'::"text")) AS "paid_tenants",
    "count"(*) FILTER (WHERE ("tenant_overview"."first_event_created" = true)) AS "tenants_with_events",
    "count"(*) FILTER (WHERE ("tenant_overview"."onboarding_completed" = true)) AS "completed_onboarding",
    "count"(*) FILTER (WHERE (("tenant_overview"."plan_name")::"text" ~~ '%Start%'::"text")) AS "start_plan_count",
    "count"(*) FILTER (WHERE (("tenant_overview"."plan_name")::"text" ~~ '%Scale%'::"text")) AS "scale_plan_count",
    "count"(*) FILTER (WHERE (("tenant_overview"."plan_name")::"text" ~~ '%Boom%'::"text")) AS "boom_plan_count",
    "count"(*) FILTER (WHERE (("tenant_overview"."plan_name")::"text" ~~ '%Enterprise%'::"text")) AS "enterprise_plan_count",
    "count"(*) FILTER (WHERE (("tenant_overview"."region")::"text" = 'Sudeste'::"text")) AS "sudeste_count",
    "count"(*) FILTER (WHERE (("tenant_overview"."region")::"text" = 'Sul'::"text")) AS "sul_count",
    "count"(*) FILTER (WHERE (("tenant_overview"."region")::"text" = 'Nordeste'::"text")) AS "nordeste_count",
    "count"(*) FILTER (WHERE (("tenant_overview"."region")::"text" = 'Norte'::"text")) AS "norte_count",
    "count"(*) FILTER (WHERE (("tenant_overview"."region")::"text" = 'Centro-Oeste'::"text")) AS "centro_oeste_count",
    "sum"("tenant_overview"."total_revenue_brl") AS "total_platform_revenue",
    "avg"("tenant_overview"."total_revenue_brl") AS "avg_revenue_per_tenant"
   FROM "public"."tenant_overview";


ALTER TABLE "public"."active_tenants_summary" OWNER TO "postgres";


COMMENT ON VIEW "public"."active_tenants_summary" IS 'Resumo estatístico dos tenants ativos';
...
COMMENT ON COLUMN "public"."tenant_usage_stats"."revenue_generated_brl" IS 'Receita gerada no período em Reais (BRL)';



CREATE OR REPLACE VIEW "public"."tenant_activity_report" AS
 SELECT "t"."id",
    "t"."slug",
    "t"."razao_social",
    "t"."status",
    ...
    COALESCE("us"."emails_sent", 0) AS "monthly_emails_sent",
        CASE
            WHEN ("t"."last_login_at" IS NULL) THEN 'never_logged_in'::"text"
            WHEN (EXTRACT(days FROM (CURRENT_TIMESTAMP - "t"."last_login_at")) <= (7)::numeric) THEN 'active'::"text"
            WHEN (EXTRACT(days FROM (CURRENT_TIMESTAMP - "t"."last_login_at")) <= (30)::numeric) THEN 'moderate'::"text"
            ELSE 'inactive'::"text"
        END AS "activity_level"
   FROM ("public"."tenant_overview" "t"
     LEFT JOIN "public"."tenant_usage_stats" "us" ON ((("t"."id" = "us"."tenant_id") AND ("us"."period_start" = ("date_trunc"('month'::"text", (CURRENT_DATE)::timestamp with time zone))::"date"))));


ALTER TABLE "public"."tenant_activity_report" OWNER TO "postgres";


COMMENT ON VIEW "public"."tenant_activity_report" IS 'Relatório de atividade dos tenants para análise de engajamento';



CREATE OR REPLACE VIEW "public"."tenant_admin_view" AS
 SELECT "t"."id",
    "t"."slug",
    "t"."cnpj",
    "t"."razao_social",
    ...
    "t"."cnae_principal",
    "t"."organizer_type_id",
    "t"."primary_segment_id",
    "t"."email_domain",
    "t"."domain_validated",
    "t"."domain_validated_at",
    "t"."status_id",
    "t"."plan_id",
    "t"."plan_expires_at",
    "t"."trial_ends_at",
    ...
    "t"."onboarding_completed",
    "t"."onboarding_current_step",
    "t"."setup_wizard_completed",
    "t"."first_event_created",
    "t"."features_enabled",
    "t"."integrations_config",
    "t"."lgpd_acceptance_date",
    ...
    "st"."name" AS "state_name"
   FROM ((((("public"."tenants" "t"
     JOIN "public"."organizer_types" "os" ON (("t"."organizer_type_id" = "os"."id")))
     JOIN "public"."business_segments" "bs" ON (("t"."primary_segment_id" = "bs"."id")))
     JOIN "public"."tenant_statuses" "ts" ON (("t"."status_id" = "ts"."id")))
     JOIN "public"."subscription_plans" "sp" ON (("t"."plan_id" = "sp"."id")))
     JOIN "public"."brazilian_states" "st" ON (("t"."state_id" = "st"."id")))
  WHERE (("t"."id" = "public"."get_current_tenant_id"()) AND ("t"."deleted_at" IS NULL));


ALTER TABLE "public"."tenant_admin_view" OWNER TO "postgres";
...
CREATE TABLE IF NOT EXISTS "public"."tenant_audit_log" (
    ...
    "lgpd_purpose" "text",
    "created_at" timestamp with time zone DEFAULT CURRENT_TIMESTAMP NOT NULL,
    CONSTRAINT "tenant_audit_log_action_check" CHECK ((("action")::"text" = ANY ((ARRAY['CREATE'::character varying, 'UPDATE'::character varying, 'DELETE'::character varying, 'LOGIN'::character varying, 'LOGOUT'::character varying, 'VIEW'::character varying, 'EXPORT'::character varying, 'IMPORT'::character varying, 'SYNC'::character varying, 'APPROVE'::character varying, 'REJECT'::character varying])::"text"[])))
);
...
CREATE TABLE IF NOT EXISTS "public"."tenant_documents" (
    ...
    "created_at" timestamp with time zone DEFAULT CURRENT_TIMESTAMP NOT NULL,
    "updated_at" timestamp with time zone DEFAULT CURRENT_TIMESTAMP NOT NULL,
    CONSTRAINT "tenant_documents_mime_check" CHECK ((("mime_type")::"text" = ANY ((ARRAY['application/pdf'::character varying, 'image/jpeg'::character varying, 'image/png'::character varying, 'image/webp'::character varying, 'application/msword'::character varying, 'application/vnd.openxmlformats-officedocument.wordprocessingml.document'::character varying])::"text"[]))),
    CONSTRAINT "tenant_documents_size_check" CHECK (("file_size_bytes" > 0)),
    CONSTRAINT "tenant_documents_status_check" CHECK ((("verification_status")::"text" = ANY ((ARRAY['pending'::character varying, 'verified'::character varying, 'rejected'::character varying, 'expired'::character varying])::"text"[]))),
    CONSTRAINT "tenant_documents_type_check" CHECK ((("document_type")::"text" = ANY ((ARRAY['contrato_social'::character varying, 'certidao_negativa'::character varying, 'cartao_cnpj'::character varying, 'procuracao'::character varying, 'alvara_funcionamento'::character varying, 'certificado_digital'::character varying, 'comprovante_endereco'::character varying, 'inscricao_municipal'::character varying, 'outros'::character varying])::"text"[])))
);
...
CREATE TABLE IF NOT EXISTS "public"."tenant_integrations" (
    ...
    "sync_status" character varying(50) DEFAULT 'never_synced'::character varying NOT NULL,
    "error_message" "text",
    "created_at" timestamp with time zone DEFAULT CURRENT_TIMESTAMP NOT NULL,
    "updated_at" timestamp with time zone DEFAULT CURRENT_TIMESTAMP NOT NULL,
    CONSTRAINT "tenant_integrations_status_check" CHECK ((("sync_status")::"text" = ANY ((ARRAY['never_synced'::character varying, 'syncing'::character varying, 'success'::character varying, 'error'::character varying, 'disabled'::character varying])::"text"[]))),
    CONSTRAINT "tenant_integrations_type_check" CHECK ((("integration_type")::"text" = ANY ((ARRAY['receita_federal'::character varying, 'sefaz'::character varying, 'correios'::character varying, 'whatsapp'::character varying, 'google_calendar'::character varying, 'outlook'::character varying, 'hubspot'::character varying, 'pipedrive'::character varying, 'rdstation'::character varying, 'mailchimp'::character varying, 'sendgrid'::character varying, 'pix_provider'::character varying, 'pagseguro'::character varying, 'mercado_pago'::character varying, 'asaas'::character varying])::"text"[])))
);
...
COMMENT ON TABLE "public"."tenant_integrations" IS 'Integrações com sistemas externos';



COMMENT ON COLUMN "public"."tenant_integrations"."api_credentials_encrypted" IS 'Credenciais criptografadas para APIs externas';
...
ALTER TABLE ONLY "public"."tenant_integrations"
    ADD CONSTRAINT "tenant_integrations_unique_type" UNIQUE ("tenant_id", "integration_type");
...
CREATE INDEX "idx_business_segments_active_sorted" ON "public"."business_segments" USING "btree" ("sort_order") WHERE ("is_active" = true);
...
CREATE INDEX "idx_organizer_types_active_sorted" ON "public"."organizer_types" USING "btree" ("sort_order") WHERE ("is_active" = true);
...
CREATE INDEX "idx_subscription_plans_active_sorted" ON "public"."subscription_plans" USING "btree" ("sort_order") WHERE ("is_active" = true);
...
CREATE INDEX "idx_tenant_integrations_sync_status" ON "public"."tenant_integrations" USING "btree" ("sync_status");
...
CREATE INDEX "idx_tenant_statuses_active_sorted" ON "public"."tenant_statuses" USING "btree" ("sort_order") WHERE ("is_active" = true);
...
CREATE OR REPLACE TRIGGER "trigger_audit_tenant_integrations" AFTER INSERT OR DELETE OR UPDATE ON "public"."tenant_integrations" FOR EACH ROW EXECUTE FUNCTION "public"."log_tenant_changes"();
...
CREATE OR REPLACE TRIGGER "trigger_tenant_integrations_updated_at" BEFORE UPDATE ON "public"."tenant_integrations" FOR EACH ROW EXECUTE FUNCTION "public"."update_updated_at_column"();
...
CREATE POLICY "brazilian_states_read_all" ON "public"."brazilian_states" FOR SELECT USING (("auth"."uid"() IS NOT NULL));
...
CREATE POLICY "business_segments_read_all" ON "public"."business_segments" FOR SELECT USING (("auth"."uid"() IS NOT NULL));
...
CREATE POLICY "organizer_types_read_all" ON "public"."organizer_types" FOR SELECT USING (("auth"."uid"() IS NOT NULL));
...
CREATE POLICY "subscription_plans_read_all" ON "public"."subscription_plans" FOR SELECT USING (("auth"."uid"() IS NOT NULL));
...
CREATE POLICY "tenant_audit_log_system_insert" ON "public"."tenant_audit_log" FOR INSERT WITH CHECK (("public"."is_super_admin"() OR ("public"."get_current_user_role"() = 'system'::"text")));



CREATE POLICY "tenant_audit_log_tenant_read" ON "public"."tenant_audit_log" FOR SELECT USING ((("tenant_id" = "public"."get_current_tenant_id"()) OR "public"."is_super_admin"()));
...
CREATE POLICY "tenant_documents_admin_modify" ON "public"."tenant_documents" USING (((("tenant_id" = "public"."get_current_tenant_id"()) AND ("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'owner'::"text"]))) OR "public"."is_super_admin"()));



CREATE POLICY "tenant_documents_tenant_access" ON "public"."tenant_documents" FOR SELECT USING ((("tenant_id" = "public"."get_current_tenant_id"()) OR "public"."is_super_admin"()));
...
CREATE POLICY "tenant_integrations_admin_modify" ON "public"."tenant_integrations" USING (((("tenant_id" = "public"."get_current_tenant_id"()) AND ("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'owner'::"text"]))) OR "public"."is_super_admin"()));



CREATE POLICY "tenant_integrations_tenant_access" ON "public"."tenant_integrations" FOR SELECT USING ((("tenant_id" = "public"."get_current_tenant_id"()) OR "public"."is_super_admin"()));
...
CREATE POLICY "tenant_plan_history_admin_insert" ON "public"."tenant_plan_history" FOR INSERT WITH CHECK (((("tenant_id" = "public"."get_current_tenant_id"()) AND ("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'owner'::"text"]))) OR "public"."is_super_admin"()));



CREATE POLICY "tenant_plan_history_tenant_read" ON "public"."tenant_plan_history" FOR SELECT USING ((("tenant_id" = "public"."get_current_tenant_id"()) OR "public"."is_super_admin"()));
...
CREATE POLICY "tenant_settings_admin_modify" ON "public"."tenant_settings" USING (((("tenant_id" = "public"."get_current_tenant_id"()) AND ("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'owner'::"text"]))) OR "public"."is_super_admin"()));



CREATE POLICY "tenant_settings_tenant_access" ON "public"."tenant_settings" FOR SELECT USING ((("tenant_id" = "public"."get_current_tenant_id"()) OR "public"."is_super_admin"()));
...
CREATE POLICY "tenant_statuses_read_all" ON "public"."tenant_statuses" FOR SELECT USING (("auth"."uid"() IS NOT NULL));
...
CREATE POLICY "tenant_usage_stats_tenant_access" ON "public"."tenant_usage_stats" USING ((("tenant_id" = "public"."get_current_tenant_id"()) OR "public"."is_super_admin"()));
...
CREATE POLICY "tenants_insert_registration" ON "public"."tenants" FOR INSERT WITH CHECK (("public"."is_super_admin"() OR ("public"."get_current_user_role"() = 'registration'::"text")));



CREATE POLICY "tenants_select_own_data" ON "public"."tenants" FOR SELECT USING ((("id" = "public"."get_current_tenant_id"()) OR "public"."is_super_admin"()));



COMMENT ON POLICY "tenants_select_own_data" ON "public"."tenants" IS 'Permite que tenants vejam apenas seus próprios dados';



CREATE POLICY "tenants_super_admin_all_access" ON "public"."tenants" USING ("public"."is_super_admin"());



COMMENT ON POLICY "tenants_super_admin_all_access" ON "public"."tenants" IS 'Super administradores têm acesso total';



CREATE POLICY "tenants_update_own_data" ON "public"."tenants" FOR UPDATE USING ((("id" = "public"."get_current_tenant_id"()) AND ("public"."get_current_user_role"() = ANY (ARRAY['admin'::"text", 'owner'::"text"]))));
...
GRANT ALL ON TABLE "public"."tenant_overview" TO "anon";
GRANT ALL ON TABLE "public"."tenant_overview" TO "authenticated";
GRANT ALL ON TABLE "public"."tenant_overview" TO "service_role";



GRANT ALL ON TABLE "public"."active_tenants_summary" TO "anon";
GRANT ALL ON TABLE "public"."active_tenants_summary" TO "authenticated";
GRANT ALL ON TABLE "public"."active_tenants_summary" TO "service_role";
...
GRANT ALL ON TABLE "public"."tenant_activity_report" TO "anon";
GRANT ALL ON TABLE "public"."tenant_activity_report" TO "authenticated";
GRANT ALL ON TABLE "public"."tenant_activity_report" TO "service_role";



GRANT ALL ON TABLE "public"."tenant_admin_view" TO "anon";
GRANT ALL ON TABLE "public"."tenant_admin_view" TO "authenticated";
GRANT ALL ON TABLE "public"."tenant_admin_view" TO "service_role";
